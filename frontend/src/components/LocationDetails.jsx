...
const LocationDetails = ({ location, onClose, embedded = false, resources }) => {
  ...
  const proximity = useMemo(() => {
    ...
    const energy = (resources.energySources || []).map((s) => ({
      ...
      type: s.type,
      distance: haversineKm(lat, lng, s.location.latitude, s.location.longitude),
    }));
    ...
    const demand = (resources.demandCenters || []).map((d) => ({
      ...
      icon: 'demand',
      name: d.name,
      capacity: d.hydrogen_demand_mt_year ? `${d.hydrogen_demand_mt_year.toLocaleString()} MT/yr` : undefined,
      type: d.type,
      distance: haversineKm(lat, lng, d.location.latitude, d.location.longitude),
    }));
    ...
    const water = (resources.waterSources || []).map((w) => ({
      ...
      icon: 'water',
      name: w.name,
      capacity: w.capacity_liters_day ? `${w.capacity_liters_day.toLocaleString()} L/day` : undefined,
      type: w.type,
      distance: haversineKm(lat, lng, w.location.latitude, w.location.longitude),
    }));
    ...
    const sortAsc = (arr) => arr.filter(e => Number.isFinite(e.distance)).sort((a, b) => a.distance - b.distance);
  ...
  const ItemRow = ({ item }) => (
    <div className="flex items-center justify-between py-2">
      ...
      <Badge className="bg-muted text-foreground whitespace-nowrap">{proximity.fmt(item.distance)}</Badge>
    </div>
  );
        ...
        {proximity && (
          <div className="space-y-3">
            ...
            <div className="bg-amber-50 p-3 rounded-lg border border-amber-200">
              <CategoryRow title="Energy Sources" icon={<Zap className="w-4 h-4 text-amber-600" />} />
              <div className="mt-2 divide-y">
                {proximity.energy.length === 0 && (
                  <div className="text-xs text-muted-foreground">No sources nearby</div>
                )}
            ...
            <div className="bg-red-50 p-3 rounded-lg border border-red-200">
              <CategoryRow title="Industrial Demand" icon={<Factory className="w-4 h-4 text-red-600" />} />
              <div className="mt-2 divide-y">
                {proximity.demand.length === 0 && (
                  <div className="text-xs text-muted-foreground">No centers nearby</div>
                )}
            ...
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
              <CategoryRow title="Water Sources" icon={<Droplets className="w-4 h-4 text-blue-600" />} />
              <div className="mt-2 divide-y">
            ...
            <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-200">
              <CategoryRow title="Nearby Highways" icon={<Route className="w-4 h-4 text-emerald-600" />} />
              <div className="mt-2 divide-y">
                {proximity.highways.length === 0 && (
                  <div className="text-xs text-muted-foreground">No highways nearby</div>
                )}
            ...
            <div className="bg-lime-50 p-3 rounded-lg border border-lime-200">
              <CategoryRow title="Pipelines" icon={<span className="text-sm">⛽</span>} />
              <div className="mt-2 divide-y">
                {proximity.pipelines.length === 0 && (
                  <div className="text-xs text-muted-foreground">No pipelines nearby</div>
                )}
