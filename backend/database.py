...
async def create_indexes():
    ...
    try:
        ...
        for collection_name in collections_with_location:
            await db_instance.database[collection_name].create_index([
                ("location.latitude", 1),
                ("location.longitude", 1)
            ])
            
        # Skip geospatial indexes for routes for now - will implement later
        # await db_instance.database['gas_pipelines'].create_index([
        #     ("route", "2dsphere")
        # ])
        
        # await db_instance.database['road_networks'].create_index([
        #     ("route", "2dsphere") 
        # ])
